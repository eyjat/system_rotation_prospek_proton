...
$tableName = "irizSA"; // set table name here
include 'db.php';
...
function rotateSalesAgent($location) {
    ...
    // Query the iriz_location_rotation table to get the last assigned agent ID for this location
    $sql = "SELECT last_assigned_agent_id FROM iriz_location_rotation WHERE location = '$location'";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Query the irizSA table to get the list of agents for this location
        $sql = "SELECT id, nameSA, phoneNum FROM irizSA WHERE locationSA = '$location'";
        $result = $conn->query($sql);
        ...
        if ($result && $result->num_rows > 0) {
            ...
            // Update the iriz_location_rotation table with the new last_assigned_agent_id
            $sql = "UPDATE iriz_location_rotation SET last_assigned_agent_id = $nextAgentIndex WHERE location = '$location'";
            $conn->query($sql);
...
$sqlInsert = "INSERT INTO irizData (prosName, prosNum, prosLocation, curDate, saName) VALUES ('$prosName', '$prosNum', '$prosLocation', '$currentDate', '$assignedAgent')";
$conn->query($sqlInsert);

// WhatsApp API integration
$whatsappApiUrl = "https://api.whatsapp.com/send?phone=6$assignedPhone&text=Hai%20saya%20$prosName,%20berminat%20nak%20beli%20iriz.";
