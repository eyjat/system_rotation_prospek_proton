...
function rotateSalesAgent($location) {
    ...
    // Query the myvi_location_rotation table to get the last assigned agent ID for this location
    $sql = "SELECT last_assigned_agent_id FROM myvi_location_rotation WHERE location = '$location'";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            // Update the myvi_location_rotation table with the new last_assigned_agent_id
            $sql = "UPDATE myvi_location_rotation SET last_assigned_agent_id = $nextAgentIndex WHERE location = '$location'";
            $conn->query($sql);
