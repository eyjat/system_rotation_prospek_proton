...
function rotateSalesAgent($location) {
    ...
    // Query the ativa_location_rotation table to get the last assigned agent ID for this location
    $sql = "SELECT last_assigned_agent_id FROM ativa_location_rotation WHERE location = '$location'";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Query the ativaSA table to get the list of agents for this location
        $sql = "SELECT id, nameSA, phoneNum FROM ativaSA WHERE locationSA = '$location'";
        $result = $conn->query($sql);
        ...
        if ($result && $result->num_rows > 0) {
            ...
            // Update the ativa_location_rotation table with the new last_assigned_agent_id
            $sql = "UPDATE ativa_location_rotation SET last_assigned_agent_id = $nextAgentIndex WHERE location = '$location'";
            $conn->query($sql);
